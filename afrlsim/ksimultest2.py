...
def train_mode():
    ...
    for episode in range(total_episodes):
        ...
        while not sim.check_simulation_over():
            ...
            # For intermediate steps, apply a small positive reward for surviving.
            reward = 0.1  
        ...
        if sim.base_hp <= 0:
            final_reward = -100  # Significant penalty for base destruction.
        else:
            # Count how many big drones were destroyed.
            big_destroyed = sum(1 for t in sim.threats if t.drone_type == "big" and t.destroyed)
            final_reward = 100 + (5 * sim.base_hp) + (10 * big_destroyed)
    ...
    scenario = {
        "observation": {
            "entities": [
                ...
                {"id": 2, "type": "INTERCEPTOR", "label": "Interceptor",
                 "location": {"latitude": 34.93, "longitude": -117.884}, "side": "blue", "ammo": ammo_value},
                {"id": 100, "type": "UAS", "label": "Big Drone 1", "drone_type": "big",
                 "location": {"latitude": 34.94, "longitude": -117.8849}, "side": "red"},
                {"id": 101, "type": "UAS", "label": "Big Drone 2", "drone_type": "big",
                 "location": {"latitude": 34.94, "longitude": -117.8639}, "side": "red"},
                {"id": 200, "type": "UAS", "label": "Small Drone 1", "drone_type": "small",
                 "location": {"latitude": 34.928, "longitude": -117.8855}, "side": "red"},
                {"id": 201, "type": "UAS", "label": "Small Drone 2", "drone_type": "small",
                 "location": {"latitude": 34.927, "longitude": -117.8865}, "side": "red"},
                {"id": 202, "type": "UAS", "label": "Small Drone 3", "drone_type": "small",
                 "location": {"latitude": 34.926, "longitude": -117.8875}, "side": "red"},
                {"id": 300, "type": "MISSILE", "label": "Enemy Missile", "drone_type": "missile",
                 "location": {"latitude": 34.95, "longitude": -117.8849}, "side": "red"}
            ]
        }
