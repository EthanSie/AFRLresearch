...
def rollout_policy(sim, q_table, actions, epsilon=0.1):
    ...
    state_key = str(state)
    # Filter actions based on applicability.
    available_actions = [action for action in actions if is_action_applicable(sim, action)]
    if not available_actions:
        # Fallback option.
        return "do_nothing"
    
    # Epsilon–greedy action selection using only available actions.
    if state_key in q_table and random.random() > epsilon:
        best_action = max(available_actions, key=lambda a: q_table[state_key].get(a, 0))
        return best_action
    else:
        return random.choice(available_actions)


# ---------------- New: Action Metrics Estimation ----------------
def estimate_action_metrics(sim, candidate_action, trials=3, q_table=None, actions=None):
    # Check for target-specific actions: if there are no available targets, return N/A metrics.
    if candidate_action == "fire_at_big" and not any(t for t in sim.threats if t.drone_type == "big" and not t.destroyed):
        return {"success_probability": "N/A",
                "average_final_base_hp": "N/A",
                "expected_damage": "N/A",
                "average_steps": "N/A",
                "average_next_hp": "N/A"}
    if candidate_action == "fire_at_small" and not any(t for t in sim.threats if t.drone_type == "small" and not t.destroyed):
        return {"success_probability": "N/A",
                "average_final_base_hp": "N/A",
                "expected_damage": "N/A",
                "average_steps": "N/A",
                "average_next_hp": "N/A"}
    if candidate_action == "fire_at_missile" and not any(t for t in sim.threats if t.drone_type == "missile" and not t.destroyed):
        return {"success_probability": "N/A",
                "average_final_base_hp": "N/A",
                "expected_damage": "N/A",
                "average_steps": "N/A",
    ...
    # Otherwise, perform the rollout trials.
    wins = 0
    total_base_hp = 0
    total_timesteps = 0
    total_next_hp = 0  # Sum of base HP right after candidate action

    for _ in range(trials):
        sim_copy = copy.deepcopy(sim)
        sim_copy.verbose = False  # Disable logging for the rollout copy.
        # Run one step with the candidate action and record next HP.
        sim_copy.run_step(action=candidate_action)
        next_hp = sim_copy.base_hp
        total_next_hp += next_hp
        # Continue simulation until termination.
        ...
        if sim_copy.base_hp > 0 and all(t.destroyed for t in sim_copy.threats):
            wins += 1
        total_base_hp += sim_copy.base_hp
        total_timesteps += sim_copy.timestep

    metrics = {
         "success_probability": wins / trials,
         "average_final_base_hp": total_base_hp / trials,
         "expected_damage": sim.base_hp - (total_base_hp / trials),
         "average_steps": total_timesteps / trials,
         "average_next_hp": total_next_hp / trials
    }
    return metrics

def is_action_applicable(sim, action):
    if action == "fire_at_big":
        return any(t for t in sim.threats if t.drone_type == "big" and not t.destroyed)
    elif action == "fire_at_small":
        return any(t for t in sim.threats if t.drone_type == "small" and not t.destroyed)
    elif action == "fire_at_missile":
        return any(t for t in sim.threats if t.drone_type == "missile" and not t.destroyed)
    # For "reload" and "do_nothing", always consider them applicable.
    return True
...
def estimate_success_probability(sim, candidate_action, trials=3, q_table=None, actions=None):
    metrics = estimate_action_metrics(sim, candidate_action, trials, q_table, actions)
    # If the metric is "N/A", return 0 (or you can choose another fallback value)
    if metrics["success_probability"] == "N/A":
        return 0
    return metrics["success_probability"]


# ---------------- Terminal Mode with Detailed Metrics ----------------
def terminal_mode():
    def clear_screen():
        os.system('cls' if os.name=='nt' else 'clear')
    
    print("=== Terminal Mode Simulation ===")
    ...
    # Create a scenario similar to demo_mode.
    ...
    while not sim.check_simulation_over():
        clear_screen()
        print("=" * 40)
        print(f"Time Step: {sim.timestep} | Base HP: {sim.base_hp}")
        print("-" * 40)
        print("Threats:")
        for t in sim.threats:
            status = "DESTROYED" if t.destroyed else f"{t.distance:.2f} km"
            print(f" - {t.label} ({t.drone_type}): {status}")
        print("-" * 40)
        print("Interceptors:")
        for idx, unit in enumerate(sim.interceptors):
            print(f" - Interceptor {idx+1}: State: {unit.current_state.value}, Ammo: {unit.ammo}")
        print("-" * 40)
        # Show current Q–table state.
        ...
        if state_key in q_table:
            ...
            for action, value in q_table[state_key].items():
                print(f"   {action:15}: {value:6.2f}")
        else:
            print("No Q–values for current state.")
        print("-" * 40)
        # Estimate metrics for each action.
        print("Estimated Action Metrics (over a few rollouts):")
        print(f"{'Action':15} {'Success':8} {'Avg Final HP':12} {'Exp Damage':11} {'Next HP':8} {'Avg Steps':10}")
        action_metrics = {}
        for act in actions:
            metrics = estimate_action_metrics(sim, act, trials=3, q_table=q_table, actions=actions)
            if metrics["success_probability"] == "N/A":
                print(f"{act:15} {'N/A':>7} {'N/A':>12} {'N/A':>11} {'N/A':>8} {'N/A':>10}")
            else:
                print(f"{act:15} {metrics['success_probability']*100:7.1f}% "
                      f"{metrics['average_final_base_hp']:12.2f} {metrics['expected_damage']:11.2f} "
                      f"{metrics['average_next_hp']:8.2f} {metrics['average_steps']:10.1f}")
        ...
        for idx, act in enumerate(actions):
            print(f" [{idx}] {act}")
        user_input = input("Enter action number (or press Enter to use best Q–value): ").strip()
        if user_input == "":
             # When defaulting to the best Q–value with applicable actions only.
            if state_key in q_table:
                applicable_actions = [act for act in actions if is_action_applicable(sim, act)]
                if applicable_actions:
                    chosen_action = max(applicable_actions, key=lambda a: q_table[state_key].get(a, 0))
                else:
            ...
            try:
                ...
                print("Invalid input. Defaulting to 'do_nothing'.")
                chosen_action = "do_nothing"

        print(f"Chosen action: {chosen_action}")
        sim.run_step(action=chosen_action)
        # Show any messages from the simulation.
    ...
    print("=" * 40)
    if sim.base_hp <= 0:
        print("Simulation Over: Defeat! The base has been destroyed.")
    else:
        print("Simulation Over: Victory! All enemy threats have been neutralized.")
    print("=" * 40)
...
if __name__ == "__main__":
    mode = input("Enter 'train', 'demo' or 'terminal': ").strip().lower()
    if mode == "train":
        train_mode()
    elif mode == "demo":
        demo_mode()
        ...
        print("Invalid mode. Please choose 'train', 'demo', or 'terminal'.")
